    ...
    "
			>
				...
				<h4 in:slide={{ duration: 600 }}>
					Does eating ice cream <span style="font-style:italic">really</span> cause serious injuries?
				</h4>
			</div>
		{/if}
		{#if step[2]}
			<div class="chart-area">
				...
				{/if}
				<p class="y-title all" in:fade class:animated={step[3]} class:initial={!step[3]}>
				...
				{#if step[4]}
					{#each yAxis as line, i}
						<div class="grid-line" style="transform:translateY({150 - (150 * line) / 300}px)">
							<p
								in:fly={{
				...
				<div class="xLabels">
					{#if step[4]}
						{#each xAxis as label, i}
							<div in:fly={{ y: 20, duration: 600, delay: i * 100 }} class="xAxisLabel">
								{label}
							</div>
						{/each}
					{/if}
...
<style>
	...
	.container .chart-area .bar-container {
		height: 160px;
		width: 200px;
		top: -160px;
		position: absolute;
	...
	.container .chart-area .bar-container .bar {
	...
	.container .chart-area .bar-container .bar .barLabel {
		position: absolute;
		top: -30px;
		width: 30px;
		text-align: center;
	...
	.container .chart-area .grid-line {
		position: absolute;
		width: 100%;
		background-color: rgba(225, 0, 255, 0.393);
		border-top: 1px dashed rgba(255, 255, 255, 0.6);
	}
	.container .chart-area .grid-line .yAxisLabel {
		position: absolute;
		color: gray;
		font-size: 0.6rem;
		width: 30px;
	...
	.container .y-title.initial.animated {
		display: flex;
		flex-wrap: wrap;
