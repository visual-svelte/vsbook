<script>
	...
	function sortBars() {
		stage = 2;
		data.blocks = data.blocks.sort((a, b) => (a.cat > b.cat ? 1 : b.cat > a.cat ? -1 : 0));
	}
		...
		<div class="initial-grid lower ">
			{#each ['apple', 'banana', 'cherry', 'pear'] as category, j}
				{#each data.bars.filter((d) => d.cat == category) as bar, i (bar.id)}
					<div
						style="transform:translate({50 + j * 50}px,{50 - i * 21}px);background-color:{colors[
							bar.cat
						][0]}"
						class="block lower"
						animate:flip
